...
int main()
{
    ...
    for (i = 10; i > 0; i--)
    {
        ...
        if ((data = (int *)malloc(sizeof(int))) == NULL)
            fprintf(stdout,"Memory could not allocated!\n");

        *data = i;

        if (dlist_ins_prev(&list, dlist_head(&list), data) != 0)
            fprintf(stdout,"Could not add element to the doubly linked list!\n");
    }
    ...
    *data = 11;
    ...
    if (dlist_remove(&list, element, (void **)&data) != 0)
        fprintf(stdout,"Could not remove the element from the linked list!\n");
